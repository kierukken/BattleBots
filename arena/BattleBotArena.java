...
//ROWBOTTOM 082023
 import java.applet.Applet;//<=remove
 import java.applet.AudioClip;//<=remove
 import java.awt.BasicStroke;//<=remove
 ...
 * @version <br>2.5 (Oct 11, 2024) - Brooks - Adding team scoring option
...
public class BattleBotArena extends JPanel implements MouseListener, MouseWheelListener, MouseMotionListener, ActionListener, Runnable {
	 ...
	 */
	public static final int RIGHT_EDGE = 1000;//1600,1100; //ROWBOTTOM - changed from 700// also arena panel width
	/**
	 * Bottom edge of the screen
	 */
	public static final int BOTTOM_EDGE = 700;//950,714; //ROWBOTTOM - changed from 500// arena panel height is this constant + TEXT_BUFFER
	/**
	...
	/**
	 * points per kill 
	 */											//TODO: Change back to 10 for S2
	public static final int 	KILL_SCORE = 5;//ROWBOTTOM Changed from 5 to promote killing
	/**
	 * survival points 
	 */											//TODO: Put back to 0.8 for S2 - maybe?
	public static final double 	POINTS_PER_SECOND = 0.1; //ROWBOTTOM 082023 increased to 0.8 reward dodging ROWBOTTOM changed from 0.1 to discourage camping
	/**
	...
	/**
	 * Enables cumulative score for entire team, not just one robot.
	 * Note: This is only implemented for when CUMULATIVE_SCORING == true
	 */
	public static final boolean TEAM_SCORING = true;//BROOKS added variable to support team scoring
	/**
	 * true - don't show score display between rounds
	 */
	public static final boolean CONTINUOUS_ROUNDS = false;  //CBP
	/**
	 ...
	 */
	public static final int 	NUM_BOTS = 30;//ROWBOTTOM NOV 6 2017 Changed to 25
	/**
	 ...
	 */											//TODO: Put back to 4 in sem 2?
	public static final double 	BOT_SPEED = 3;//Math.floor(RIGHT_EDGE/400);//ROWBOTTTOM changed from 3 to accomodate larger screensROWBOTTOM changed from 2.5 to fix game balance and bots getting hung up on tombstones
	/**
	 * Bullet speed in pixels/frame
	 */											//TODO: Put back to 10 in sem 2?
	public static final double 	BULLET_SPEED = 6;//Math.floor(RIGHT_EDGE/400);//ROWBOTTTOM changed from 6 to accomodate larger screens
	/**
	 ...
	 */
	private final int NAME_FONT = 11; //ROWBOTTOM 82023 increased from 10
	/**
	 * Size of the stats font (stats displayed at end of each round)
	 */
	private final int STATS_FONT = 16;//ROWBOTTOM 82023 increased from 15
	/**
	...
	/**
	 * If set to true, the arena will display Ammo amounts during the game as long as
	 * showTeams is false
	 */
	...
	/**
	 * A Map storing the each team score - only relevant if TEAM_SCORING == true
	 */
	private Map<String, Double> teamScores = new HashMap<String, Double>();
      ...
      //offets made to move
			...
			for (int i = 0; i < NUM_BOTS; i++)
			{
				...
				if (botsInfo[i] != null && CUMULATIVE_SCORING && round > 1)
						if (TEAM_SCORING) //BROOKS - Disable the cumulative addition if scoring by teams 
							newBotsInfo[i].setCumulativeScore(botsInfo[i].getCumulativeScore());
						else
		...
		if (state == TEST_MODE)
		{
			sendMessage(-1,"Battle Bots, by Sam Scott and modified by Nathan Rowbottom");
			sendMessage(-1,"------------------------------------------------");
	...
	public void actionPerformed(ActionEvent ace)
	{
		...
		if (state == GAME_ON && countDown <= 0 || state == TEST_MODE && ready)
		{
			...
			if (state != TEST_MODE && (timePlayed >= TIME_LIMIT || botsLeft <= 1))
			{
				...
				//Mangage team scores if enabled
				if (CUMULATIVE_SCORING && TEAM_SCORING){
					//Calculate scores for all teams
					for (int i = 0; i < NUM_BOTS; i++) {
						String teamName = botsInfo[i].getTeamName();
						Double score = teamScores.get(teamName);
						if (score != null) {//team not in map, add to map and add score
							score += botsInfo[i].getScore();
							teamScores.replace(teamName, score );
						}
						else { //team in map, add score to existing value
							score = botsInfo[i].getScore();
							// System.out.println("Setting initial score for " + bots[i].getTeamName() + " to "+ botsInfo[i].getScore() );
							teamScores.put(bots[i].getTeamName(), score );

						}				 
					}

					//Set cumulative score to team score for each bot
					for (int i = 0; i < NUM_BOTS; i++) {
						botsInfo[i].setCumulativeScore(teamScores.get(botsInfo[i].getTeamName()));;
					}
				...
				if (totalOut >= NUM_BOTS-1) // is this the last round?
				{
					...
					if (CUMULATIVE_SCORING){ // different message depending on scoring type
						if (TEAM_SCORING) 
							sendMessage(-1,"Round "+round+" complete. "+sortedBots[0].getTeamName()+" is leading.");
						else
	...
	private BotInfo[] sortedBotInfoArray(boolean descending)
	{
		...
		for (int i=NUM_BOTS-1; i>0; i--)
			for (int j=1; j<=i; j++)
			{
				...
				double score2; 
				if (TEAM_SCORING) {
					score1 = newInfos[j].getCumulativeScore();
					score2 = newInfos[j-1].getCumulativeScore();

				}else {
					score1 = newInfos[j].getScore()+newInfos[j].getCumulativeScore();
					score2 = newInfos[j-1].getScore()+newInfos[j-1].getCumulativeScore();
				}
	...
	private void printStats(Graphics g)
	{
		...
		if (state != WAIT_TO_START)
		{
			...
			for (int i=0; i<NUM_BOTS; i++)
			{
				double score;
				if (CUMULATIVE_SCORING && TEAM_SCORING)
					score = newInfos[i].getCumulativeScore();
				else
					score = newInfos[i].getScore()+newInfos[i].getCumulativeScore();

				String output = pad(newInfos[i].getName(), 8, false) + " " + pad(newInfos[i].getTeamName(),8, false)+" ";
				output += (newInfos[i].isOut()?"     ":pad(df.format(newInfos[i].getScore()),5, true))+"  "+pad(df.format(score),5, true)+" ";
				if (!newInfos[i].isOut())
				{
		...
		if (ready) // avoid race condition on startup
		{
			...
			else if (state == GAME_OVER)
			{
				...
				if (CUMULATIVE_SCORING) { 
					if (TEAM_SCORING)
						g.drawString(botsInfo[winnerID].getTeamName()+" Leading After "+round+" Round"+(round>1?"s":"")+". Click This Bar to Continue.",10,BOTTOM_EDGE - 10);
					else
			...
			else if (state == WINNER)
			{
				...
				if (CUMULATIVE_SCORING) 
					if (TEAM_SCORING)
						g.drawString(botsInfo[winnerID].getTeamName()+" Wins after "+round+" Round"+(round>1?"s":"")+"! Click This Bar to Restart.",10,BOTTOM_EDGE - 10);
					else
			...
			else if (state == WAIT_TO_START)
			{
				...
				if (CUMULATIVE_SCORING) 
					if (TEAM_SCORING)
						g.drawString("- team scores accumulate from round to round",10,y);
					else 
